...
写入流水线是流式写入的核心路径（默认 `-write-buffer-ms 5`，`0` 可关闭）：

- **写入走单个后台 goroutine**：请求 goroutine 只负责反序列化（JSON / Line
  Protocol / 二进制），解码出的点立即交给后台写入器，解码与入库并行，
  反序列化不再阻塞引擎写入、写入也不阻塞解码。
- **按表合并大批**：一个合并周期内，同一张表的多个请求/多个流式分批先拼成
  大批再调用引擎 `WriteBatch`（每批以 `-write-batch-size` 为上限分批），
  减少引擎 WAL 锁获取次数，引擎单锁下大批明显快于小批。
- **写后立即可查**：查询、单点写、关闭前自动 flush 缓冲；数据可见性语义与
  立即写一致，只是写入最多有 ≤5ms 的缓冲延迟（进程崩溃时最多丢 5ms）。
- **背压**：缓冲点数达到 `batch-size × 32` 时退化为同步写，不无限积压内存。

**实测结论（本机，HTTP 批量 10 点/请求、32 并发）**：

| 配置 | 吞吐 |
|---|---|
| 无流水线（`-write-buffer-ms 0`，立即写） | ~44K pts/s（瓶颈为 HTTP 请求往返/连接池） |
| 流水线（100ms 合并） | ~44K pts/s（无净增加） |
| 客户端合并（1000 点/批） | ~480K pts/s（25×） |

在「小请求高频接入」的 HTTP 受限场景，吞吐瓶颈是请求数与往返，流水线无法
减少请求数、增益有限；在「客户端无法批量合并、单连接流式上传大请求」的
场景（`/api/v1/batch` 流式 JSON / Line Protocol / 二进制批量），流水线让
解码与入库并行并按表合并，吞吐提升明显，因此默认开启。

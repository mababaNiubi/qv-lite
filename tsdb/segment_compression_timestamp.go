...
//
// Binary layout per strategy (1-byte header, 4 high bits = encoding type,
// 4 low bits = log10 of the scaling divisor):
//
//   RLE (timeCompressedRLE):
//     [0]      header: (timeCompressedRLE << 4) | log10(divisor)
//     [1:9]    first: uint64 BE — the first timestamp
//     [9..]    delta: uvarint — scaled delta (original delta / divisor)
//     [..]     count: uvarint — number of timestamps
//
//   simple8b (timeCompressedPackedSimple):
//     [0]      header: (timeCompressedPackedSimple << 4) | log10(divisor)
//     [1:9]    first_delta: uint64 BE — uncompressed delta from t[0] to t[1]
//     [9..]    N × uint64 BE — simple8b-encoded deltas for t[2..N]
//
//   uncompressed (timeUncompressed):
//     [0]      header: timeUncompressed << 4
//     [1..]    N × uint64 BE — raw timestamps (no delta encoding)
//
import (
	"encoding/binary"
	"fmt"
...
func (e *TimeEncoder) Length() int {
	return len(e.ts)
}

func (e *TimeEncoder) GetMinTime() int64 {
	return e.minTime
}
...
func (e *TimeEncoder) reduce() (max, divisor uint64, rle bool, deltas []uint64) {
...
func (e *TimeEncoder) Bytes() ([]byte, error) {
	if len(e.ts) == 0 {
...
func (e *TimeEncoder) encodePacked(div uint64, dts []uint64) ([]byte, error) {
...
func (e *TimeEncoder) encodeRaw() ([]byte, error) {
	sz := 1 + len(e.ts)*8
	if cap(e.bytes) < sz {
		e.bytes = make([]byte, sz)
	}
...
func (e *TimeEncoder) encodeRLE(first, delta, div uint64, n int) ([]byte, error) {
